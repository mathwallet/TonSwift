...
func getRefsDescriptor(refs: [ConnectCell], level: UInt32, type: CellType) -> UInt8 {
    let typeFactor: UInt8 = type != .ordinary ? 1 : 0
    return UInt8(refs.count) + typeFactor * 8 + UInt8(level) * 32
}

func getBitsDescriptor(bits: ConnectBitstring) -> UInt8 {
    let len = bits.length
    return UInt8(ceil(Double(len) / 8) + floor(Double(len) / 8))
}

func readCell(reader: ConnectSlice, sizeBytes: Int) throws -> (exotic: Bool, bits: ConnectBitstring, refs: [UInt64]) {
    ...
    var bits = ConnectBitstring.empty
    if dataBytesize > 0 {
        if paddingAdded {
...
func calcCellSize(cell: ConnectCell, sizeBytes: Int) -> Int {
    return 2 /* D1+D2 */ + Int(ceil(Double(cell.bits.length) / 8.0)) + cell.refs.count * sizeBytes
}

func deserializeBoc(src: Data) throws -> [ConnectCell] {
    let boc = try ConnectBoc(data: src)
    let reader = ConnectSlice(data: boc.cellData)
    
    var cells: [(bits: ConnectBitstring, refs: [UInt64], exotic: Bool, result: ConnectCell?)] = []
    ...
    for i in stride(from: cells.count - 1, through: 0, by: -1) {
        if cells[i].result != nil {
            throw TonError.otherError("Impossible")
        }
        
        var refs: [ConnectCell] = []
        ...
        cells[i].result = try ConnectCell(exotic: cells[i].exotic, bits: cells[i].bits, refs: refs)
    }

    var roots: [ConnectCell] = []
...
func writeCellToBuilder(cell: ConnectCell, refs: [UInt64], sizeBytes: Int, to: ConnectBuilder) throws -> ConnectBuilder {
...
func serializeBoc(root: ConnectCell, idx: Bool, crc32: Bool) throws -> Data {
    ...
    // Serialize
    var builder = ConnectBuilder(capacity: totalSize)
    ...
    if res.count != totalSize / 8 {
        throw TonError.otherError("Internal error")
    }
    
    return res
}

func topologicalSort(src: ConnectCell) throws -> [(cell: ConnectCell, refs: [UInt64])] {
    var pending: [ConnectCell] = [src]
    var allCells = [String: (cell: ConnectCell, refs: [String])]()
    ...
    func visit(hash: String) throws {
        ...
        if tempMark.contains(hash) {
            throw TonError.otherError("Not a DAG")
        }
    ...
    var result: [(cell: ConnectCell, refs: [UInt64])] = []
    for ent in sorted {

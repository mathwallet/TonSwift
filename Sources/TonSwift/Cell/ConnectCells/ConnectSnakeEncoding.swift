...
extension ConnectSlice {
    ...
    public func loadSnakeString() throws -> String {
        guard let str = String(data: try self.loadSnakeData(), encoding: .utf8) else {
            throw TonError.otherError("Cannot read slice to string")
        }
        return str;
    }
    ...
    public func loadSnakeData() throws -> Data {
        ...
        if self.remainingBits % 8 != 0 {
            throw TonError.otherError("Invalid string length: \(self.remainingBits)")
        }
        if self.remainingRefs != 0 && self.remainingRefs != 1 {
            throw TonError.otherError("Invalid number of refs: \(self.remainingRefs)")
        }
        if self.remainingRefs == 1 && (BitsPerCell - self.remainingBits) > 7 {
            throw TonError.otherError("Invalid string length: \(self.remainingBits / 8)")
        }
    ...
    public func writeSnakeData(_ src: Data) throws -> Self {
        if src.count > 0 {
            ...
            if src.count > bytes {
                ...
                let cell = try (try ConnectBuilder().writeSnakeData(t)).endCell()
...
extension String {
    /// Encodes a String into a ConnectCell
    public func toTonCell() throws -> ConnectCell {
        return try ConnectBuilder().writeSnakeData(Data(self.utf8)).endCell();
    }
}

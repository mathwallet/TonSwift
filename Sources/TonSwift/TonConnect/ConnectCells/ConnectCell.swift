...
/// Number of bits that fits into a ConnectCell
public let BitsPerCell = 1023

/// Number of refs that fits into a ConnectCell
...
/// Metrics of a cell or a ConnectBuilder
...
/// TON cell
public struct ConnectCell: Hashable {
    
    private let basic: BasicCell
    
    public var type: CellType { basic.type }
    public var bits: ConnectBitstring { basic.bits }
    public var refs: [ConnectCell] { basic.refs }
    ...
    /// Empty ConnectCell with no bits and no refs.
    public static let empty = ConnectCell()
    
    /// Initializes a new cell with. Exotic cells are parsed and resolved using their contents.
    public init(
        exotic: Bool = false,
        bits: ConnectBitstring = ConnectBitstring.empty,
        refs: [ConnectCell] = []
    ) throws {
        ...
        if exotic {
            ...
            if refs.count > RefsPerCell {
                throw TonError.otherError("Invalid number of references")
            }
            if bits.length > BitsPerCell {
                throw TonError.otherError("Bits overflow: \(bits.length) > \(BitsPerCell)")
            }
    ...
    public init() {
        self.basic = BasicCell(type: .ordinary, bits: ConnectBitstring.empty, refs: [])
        self.mask = LevelMask()
    }
    
    /// Initializes a new cell with plain bytestring. This does not parse Bag-of-Cells (BoC), but uses provided data as a ConnectBitstring (byte-aligned).
    /// Throws if data contains more than 1023 bits.
    public init(data: Data) throws {
        try self.init(bits: ConnectBitstring(data: data))
    }
    ...
    */
    public static func fromBoc(src: Data) throws -> [ConnectCell] {
    ...
    */
    public static func fromBase64(src: String) throws -> ConnectCell {
        guard let data = Data(base64Encoded: src) else {
            throw NSError()
        }

        let parsed = try ConnectCell.fromBoc(src: data)
        if parsed.count != 1 {
            throw TonError.otherError("Deserialized more than one ConnectCell")
        }
    ...
    */
    ...
    /// Convert cell to ConnectSlice so it can be parsed.
    /// Same as `toSlice`.
    public func beginParse(allowExotic: Bool = false) throws -> ConnectSlice {
        if isExotic && !allowExotic {
            throw TonError.otherError("Exotic cells cannot be parsed");
        }
        
        return ConnectSlice(cell: self)
    }
    
    /// Convert cell to ConnectSlice so it can be parsed.
    /// Same as `beginParse`.
    public func toSlice() throws -> ConnectSlice {
    ...
    public func toBuilder() throws -> ConnectBuilder {
        return try ConnectBuilder().store(slice: toSlice())
    }
    /**
...
fileprivate struct BasicCell: Hashable {
    let type: CellType
    let bits: ConnectBitstring
    let refs: [ConnectCell]
    
    /// Parse the exotic cell
    static func exotic(bits: ConnectBitstring, refs: [ConnectCell]) throws -> Self {
        let reader = ConnectSlice(bits: bits)
        ...
        case 2:
            throw TonError.otherError("Library ConnectCell must be loaded automatically")
        ...
        default:
            throw TonError.otherError("Invalid exotic ConnectCell type: \(typeInt)")
        }
    ...
    func precompute() throws -> (mask: LevelMask, hashes: [Data], depths: [UInt32]) {
        ...
        for levelI in 0...levelMask.level {
            ...
            // Bits
            var currentBits: ConnectBitstring
            if hashI == hashIOffset {
                if !(levelI == 0 || type == .prunedBranch) {
                    throw TonError.otherError("Invalid")
                }
                currentBits = bits
            } else {
                if !(levelI != 0 && type != .prunedBranch) {
                    throw TonError.otherError("Invalid: \(levelI), \(type)")
                }
                currentBits = ConnectBitstring(data: hashes[Int(hashI - hashIOffset) - 1], unchecked: (offset: 0, length: 256))
            }
...
func getRepr(bits: ConnectBitstring, refs: [ConnectCell], level: UInt32, type: CellType) throws -> Data {
    // Allocate
...
func exoticPruned(bits: ConnectBitstring, refs: [ConnectCell]) throws -> ExoticPruned {
    let reader = ConnectSlice(bits: bits)

    let type = try reader.loadUint(bits: 8)
    if type != 1 {
        throw TonError.otherError("Pruned branch ConnectCell must have type 1, got \(type)")
    }

    if refs.count != 0 {
        throw TonError.otherError("Pruned Branch ConnectCell can't has refs, got \(refs.count)");
    }
    ...
    if bits.length == 280 {
        ...
        if mask.level < 1 || mask.level > 3 {
            throw TonError.otherError("Pruned Branch ConnectCell level must be >= 1 and <= 3, got \(mask.level)/\(mask.value)");
        }

        // Read pruned
        let size = 8 + 8 + (mask.apply(level: mask.level - 1).hashCount * (256 /* Hash */ + 16 /* Depth */))
        if bits.length != size {
            throw TonError.otherError("Pruned branch ConnectCell must have exactly size bits, got \(bits.length)");
        }
    }
...
func resolvePruned(bits: ConnectBitstring, refs: [ConnectCell]) throws -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
...
func resolveMerkleProof(bits: ConnectBitstring, refs: [ConnectCell]) throws -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
...
func resolveMerkleUpdate(bits: ConnectBitstring, refs: [ConnectCell]) throws -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
...
@discardableResult
func exoticMerkleUpdate(bits: ConnectBitstring, refs: [ConnectCell]) throws -> (proofDepth1: UInt32, proofDepth2: UInt32, proofHash1: Data, proofHash2: Data) {
    let reader = ConnectSlice(bits: bits)
    ...
    if bits.length != size {
        throw TonError.otherError("Merkle Update ConnectCell must have exactly (8 + (2 * (256 + 16))) bits, got \(bits.length)")
    }

    if refs.count != 2 {
        throw TonError.otherError("Merkle Update ConnectCell must have exactly 2 refs, got \(refs.count)")
    }

    let type = try reader.loadUint(bits: 8)
    if type != 4 {
        throw TonError.otherError("Merkle Update ConnectCell type must be exactly 4, got \(type)")
    }
    ...
    if proofDepth1 != refs[0].depth(level: 0) {
        throw TonError.otherError("Merkle Update ConnectCell ref depth must be exactly \(proofDepth1), got \(refs[0].depth(level: 0))")
    }

    if proofHash1 != refs[0].hash(level: 0) {
        throw TonError.otherError("Merkle Update ConnectCell ref hash must be exactly \(proofHash1.hexString()), got \(refs[0].hash(level: 0).hexString())")
    }

    if proofDepth2 != refs[1].depth(level: 0) {
        throw TonError.otherError("Merkle Update ConnectCell ref depth must be exactly \(proofDepth2), got \(refs[1].depth(level: 0))")
    }

    if proofHash2 != refs[1].hash(level: 0) {
        throw TonError.otherError("Merkle Update ConnectCell ref hash must be exactly \(proofHash2.hexString()), got \(refs[1].hash(level: 0).hexString())")
    }
...
@discardableResult
func exoticMerkleProof(bits: ConnectBitstring, refs: [ConnectCell]) throws -> (proofDepth: UInt32, proofHash: Data) {
    let reader = ConnectSlice(bits: bits)
    ...
    if bits.length != size {
        throw TonError.otherError("Merkle Proof ConnectCell must have exactly (8 + 256 + 16) bits, got \(bits.length)")
    }
    if refs.count != 1 {
        throw TonError.otherError("Merkle Proof ConnectCell must have exactly 1 ref, got \(refs.count)")
    }

    // Check type
    let type = try reader.loadUint(bits: 8)
    if type != 3 {
        throw TonError.otherError("Merkle Proof ConnectCell must have type 3, got \(type)")
    }
    ...
    if proofDepth != refDepth {
        throw TonError.otherError("Merkle Proof ConnectCell ref depth must be exactly \(proofDepth), got \(refDepth)")
    }

    if proofHash != refHash {
        throw TonError.otherError("Merkle Proof ConnectCell ref hash must be exactly \(proofHash.hexString()), got \(refHash.hexString())")
    }

...
public class ConnectBuilder {
    ...
    public private(set) var refs: [ConnectCell]
    ...
    /// Initialize the ConnectBuilder with a given capacity.
    /// Note: ConnectBuilder can be used to construct larger bitstrings, not only 1023-bit Cells. E.g. to build a BoC.
    ...
    public convenience init(_ bits: ConnectBitstring) throws {
        self.init()
        try self.store(bits: bits)
    }
    
    private init(capacity: Int, buffer: Data, length: Int, refs: [ConnectCell]) {
        self.capacity = capacity
        self._buffer = buffer
        self._length = length
        self.refs = refs
    ...
    public func clone() -> ConnectBuilder {
        return ConnectBuilder(capacity: capacity, buffer: _buffer, length: _length, refs: refs)
    }
    
    /// Completes ConnectCell
    public func endCell() throws -> ConnectCell {
        let bits = ConnectBitstring(data: _buffer, unchecked:(offset: 0, length: _length))
        return try ConnectCell(bits: bits, refs: refs)
    }
    
    /// Same as `endCell`
    public func asCell() throws -> ConnectCell {
        return try endCell()
    }

    /// Converts ConnectBuilder into ConnectBitstring
    public func bitstring() -> ConnectBitstring {
        return ConnectBitstring(data: _buffer, unchecked:(offset: 0, length: _length))
    }
    ...
    public func alignedBitstring() throws -> Data {
        if !aligned {
            throw TonError.otherError("ConnectBuilder buffer is not byte-aligned")
        }
    ...
    /**
     Store reference
     - parameter cell: cell or ConnectBuilder to store
     - returns this ConnectBuilder
     */
    @discardableResult
    public func store(ref cell: ConnectCell) throws -> Self {
        if refs.count >= 4 {
            throw TonError.otherError("Too many references")
        }
        refs.append(cell)
        return self
    }
    @discardableResult
    public func store(ref builder: ConnectBuilder) throws -> Self {
     ...
     */
    @discardableResult
    public func storeMaybe(ref cell: ConnectCell?) throws -> Self {
        if let cell = cell {
            try store(bit: true)
            try store(ref: cell)
        } else {
    ...
    @discardableResult
    public func storeMaybe(ref builder: ConnectBuilder?) throws -> Self {
    ...
    /**
     Store slice it in this ConnectBuilder
     - parameter src: source slice
     */
    @discardableResult
    public func store(slice: ConnectSlice) throws -> Self {
        let c = slice.clone()
    ...
    /**
     Store slice in this ConnectBuilder if not null
     - parameter src: source slice
     */
    public func storeMaybe(slice: ConnectSlice?) throws {
        if let slice = slice {
            try store(bit: true)
    ...
    public func store(bit: Bit, repeat count: Int) throws -> Self {
        if count < 0 { throw TonError.otherError("In store(bit:repeat:) repeat count must be non-negative.") }
        for _ in 0..<count {
            try store(bit: bit)
        }
        return self
    ...
    @discardableResult
    public func store(bits: ConnectBitstring) throws -> Self {
    ...
    public func store(binaryString: String) throws -> Self {
        for s in binaryString {
            if s != "0" && s != "1" {
                throw TonError.otherError("ConnectBitstring must contain only 0s and 1s. Invalid character: \(s)")
            }
    ...
    */
    ...
    public func store(biguint value: BigUInt, bits: Int) throws -> Self {
        ...
        if aligned {
            ...
            if bits == 8 {
                let v = Int(value)
                if v < 0 || v > 255 {
                    throw TonError.otherError("Value is out of range for \(bits) bits. Got \(value)")
                }
        ...
        if bits == 0 {
            if value != 0 {
                throw TonError.otherError("value is not zero for \(bits) bits. Got \(value)")
            } else {
                return self
            }
        }
        ...
        if v < 0 || v >= vBits {
            throw TonError.otherError("BitLength is too small for a value \(value). Got \(bits)")
        }
    ...
    func store(bigint value: BigInt, bits: Int) throws -> Self {
        var v = value
        if bits < 0 {
            throw TonError.otherError("Invalid bit length. Got \(bits)")
        }
        
        if bits == 0 {
            if v != 0 {
                throw TonError.otherError("Value is not zero for \(bits) bits. Got \(v)")
            } else {
                return self
            }
        }
        ...
        if bits == 1 {
            if v != -1 && v != 0 {
                throw TonError.otherError("Value is not zero or -1 for \(bits) bits. Got \(v)")
            } else {
                try store(bit: v == -1)
                return self
            }
        ...
        let vBits = 1 << (bits - 1)
        if v < -vBits || v >= vBits {
            throw TonError.otherError("Value is out of range for \(bits) bits. Got \(v)")
        }
    ...
    func store(varuint v: BigUInt, limit: Int) throws -> Self {
        let maxsize = limit - 1
        if maxsize < 0 {
            throw TonError.otherError("Invalid limit. Got \(limit) < 1")
        }
        if v.bitWidth > maxsize * 8 {
            throw TonError.otherError("Invalid maxsize bitWidth")
        }
    ...
    private func checkCapacity(_ bits: Int) throws {
        if availableBits < bits || bits < 0 {
            throw TonError.otherError("ConnectBuilder overflow: need to write \(bits), but available \(availableBits)")
        }
    }

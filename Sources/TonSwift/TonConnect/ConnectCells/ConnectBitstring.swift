...
public struct ConnectBitstring: Hashable {
    
    public static let empty = ConnectBitstring(data: .init(), unchecked: (offset: 0, length: 0))
    ...
    /// Constructs ConnectBitstring from a buffer with specified offset and length and checs for consistency.
    public init(data: Data, offset: Int, length: Int) throws {
        guard offset >= 0 else {
            throw TonError.otherError("Offset cannot be negative")
        }
        guard length >= 0 else {
            throw TonError.otherError("Length cannot be negative")
        }
        guard (offset + length) <= data.count * 8 else {
            throw TonError.otherError("Offset and length out of bounds for the data")
        }
        self._data = data
        self._offset = offset
        self._length = length
    ...
    /**
     Constructs ConnectBitstring from a buffer with specified offset and length without checking consistency.
    ...
    /// Constructs ConnectBitstring from a buffer of data
    ...
    /// Constructs ConnectBitstring from a binary string of 1s and 0s.
    public init(binaryString: String) throws {
        let cell = try ConnectBuilder().store(binaryString: binaryString).endCell()
        self = cell.bits
    }
    ...
    public func at(_ index: Int) throws -> Bit {
        guard index <= _length && index >= 0 else {
            throw TonError.otherError("index error")
        }
     ...
     */
    public func substring(offset: Int, length: Int) throws -> ConnectBitstring {
        // Corner case of empty string
        if length == 0 && offset == _length {
            return ConnectBitstring.empty
        }
        
        try checkOffset(offset: offset, length: length)
        
        return ConnectBitstring(data: _data, unchecked:(offset: _offset + offset, length: length))
    }
    ...
    ///
    /// Note that a ConnectBitstring may be backed by a shared buffer with non-aligned offset;
    /// in such case the alignment is checked for the sum of internal offset and provided offset.
    ///
    /// TODO: might be useful to re-align ConnectBitstring in such case and only require that `length` is byte-aligned.
    ...
    /// Drops first `n` bits from the ConnectBitstring.
    public func dropFirst(_ n: Int) throws -> ConnectBitstring {
        return try substring(offset: n, length: self.length - n)
    }
    
    /// Formats the ConnectBitstring as a hex-encoded string with a `_` trailing symbol indicating `10*` padding to 4-bit alignment.
    ...
    /// Formats the ConnectBitstring in binary digits.
    ...
    private func checkOffset(offset: Int, length: Int) throws {
        if offset >= _length || offset < 0 || offset + length > _length {
            throw TonError.otherError("offset error")
        }
    }
    
    public func padLeft(_ n: Int = 0) -> ConnectBitstring {
        let cap = max(n, self.length)
        let b = ConnectBuilder(capacity: cap)
        try! b.store(bit: 0, repeat: cap - self.length)
        try! b.store(bits: self)
        return try! b.endCell().bits
    }
    
    /// Pads ConnectBitstring with `10*` bits.
    public func bitsToPaddedBuffer() -> Data {
        let builder = ConnectBuilder(capacity: (self.length + 7) / 8 * 8)
...
/// ConnectBitstring implements lexicographic comparison.
extension ConnectBitstring: Comparable {
...
extension ConnectBitstring: Equatable {
    
    /**
     Checks for equality
    - parameter lhs: ConnectBitstring
    - parameter rhs: ConnectBitstring
    - returns true if the ConnectBitstrings are equal, false otherwise
     */
    public static func == (lhs: ConnectBitstring, rhs: ConnectBitstring) -> Bool {

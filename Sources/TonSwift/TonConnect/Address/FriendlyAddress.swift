...
public struct FriendlyAddress: Codable, Hashable {
    ...
    public init(string: String) throws {
        ...
        guard let data = Data(base64Encoded: string) else {
            throw TonError.otherError("ConnectAddress is not correctly encoded in Base64")
        }
    ...
    public init(data: Data) throws {
        ...
        if data.count != 36 {
            throw TonError.otherError("Unknown ConnectAddress type: byte length is not equal to 36")
        }
        ...
        if tag != bounceableTag && tag != nonBounceableTag {
            throw TonError.otherError("Unknown ConnectAddress tag")
        }
        ...
        self.address = ConnectAddress(workchain: wc, hash: hash)
    }
    
    public init(address: ConnectAddress, testOnly: Bool = false, bounceable: Bool = BounceableDefault) {
